...
func (s *Series) Argsort() *Series {
	...
	values := make([]int, len(pos))
	copy(values, pos)
	return fromColumn(s.name, column.NewInt(values, nil), s.index.Clone())
}
...
func (s *Series) Rank(opts ...RankOption) (*Series, error) {
	...
	for i := 0; i < s.Len(); i++ {
		...
		if !ok {
			return nil, fmt.Errorf("%w: rank on non-numeric value %T", errs.ErrTypeMismatch, s.col.Value(i))
		}
	...
	return floatColumnSeries(s.name, data, mask, s.index), nil
}
...
func (s *Series) shiftCombine(periods int, f func(cur, prev float64) float64) (*Series, error) {
	...
	floats, present, err := numericBuffer(s)
	if err != nil {
		return nil, err
	}
	n := s.Len()
	...
	for i := 0; i < n; i++ {
		prev := i - periods
		if prev < 0 || prev >= n || !present[i] || !present[prev] {
			mask[i] = true
			continue
		}
		data[i] = f(floats[i], floats[prev])
	}
	return floatColumnSeries(s.name, data, mask, s.index), nil
}
...
// mapNumeric applies a float function, keeping typed integer storage for
// integer input when every result stays integral.
func (s *Series) mapNumeric(f func(x float64) float64) (*Series, error) {
	...
	for i := 0; i < n; i++ {
		...
		out[i] = f(floats[i])
		if keepInt && out[i] != math.Trunc(out[i]) {
			keepInt = false
		}
	}
	if keepInt {
	...
	return floatColumnSeries(s.name, out, mask, s.index), nil
}
...
func (s *Series) Shift(periods int) *Series {
	...
	col, _ := s.col.Take(pos)
	return &Series{name: s.name, col: col, index: s.index.Clone()}
}
